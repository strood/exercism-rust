...
#[test]
fn test_grades_does_not_expose_internal_data() {
    let mut school = school::School::new();
    school.add(1, "Alice");
    let mut grades = school.grades();
    grades.push(2);
    assert_eq!(school.grades(), vec![1]); // should not include 2
}

#[test]
fn test_grade_does_not_expose_internal_data() {
    let mut school = school::School::new();
    school.add(1, "Alice");
    let mut students = school.grade(1);
    students.push("Bob".to_string());
    assert_eq!(school.grade(1), vec!["Alice"]); // should not include "Bob"
}
